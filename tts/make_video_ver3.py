...
def main():
    ...
    # Validate that every WAV file in the requested chapter range exists
    ...
    if missing_wav_files:
        ...
        for ch, wav_file in missing_wav_files:
            print(f"  Chapter {ch}: {wav_file}")

        print(
            "\nVideo generation stopped. "
            "Please generate all missing audio files first."
        )
        sys.exit(1)
    ...
    # At this point every requested WAV file has already been verified.
    for ch in range(start_ch, end_ch + 1):
        wav_file = audio_dir / f"{ch}.wav"
